  ...
  <div class="pb-2">
    ...
    <div class="grid grid-cols-2 gap-3 xl:grid-cols-5">
      <template v-for="(product, index) in products" :key="product.id">
        <div v-if="parseInt(index) < 10" class="bg-white px-2 py-3 rounded-lg text-red-primary">
          <NuxtLink :to="product.category_link"
            ...
            <img class="max-w-[90%] max-h-[90%] group-hover:max-w-full group-hover:max-h-full transition-all"
              :src="`${urls.BACKEND_URL}${product.series_img}`" alt="iphone 12 Lock 64Gb" />
            <div v-if="product.new_price && product.old_price"
          ...
          <NuxtLink :to="product.category_link" class="font-semibold block text-center leading-5 -mt-1">{{ product.name }}
          </NuxtLink>
          <div
            ...
            <span class="text-base text-gray-500 line-through mx-0.5 md:mx-1.5" v-if="product.old_price">
