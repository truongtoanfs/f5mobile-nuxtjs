  ...
  <div class="pb-2">
    <div class="py-5 flex flex-col xl:flex-row xl:justify-between xl:items-center xl:py-1">
      <div class="flex flex-col items-center xl:flex-row">
        ...
        <NuxtLink :to="productCategoriesObject?.link"
      ...
      <div
        ...
        <NuxtLink v-for="linkObject in productCategoriesObject?.childs" :key="linkObject.title" :to="linkObject.link"
    ...
    <div class="grid grid-cols-2 gap-3 xl:grid-cols-5">
      <template v-for="(product, index) in productsList" :key="product.id">
        <div v-if="index < 10" class="bg-white px-2 py-3 rounded-lg text-red-primary">
          <NuxtLink :to="product.path" class="flex items-center justify-center h-48 group overflow-hidden relative">
            <img class="max-w-[90%] max-h-[90%] group-hover:max-w-full group-hover:max-h-full transition-all"
              :src="getImageUrl(product.avatar)" alt="iphone 12 Lock 64Gb" />
            <div
          ...
          <NuxtLink :to="product.path" class="font-semibold block text-center leading-5 -mt-1">{{ product.title
          }}</NuxtLink>
          <div
            ...
            <span class="text-base text-gray-500 line-through mx-0.5 md:mx-1.5">
              {{ formatNumberToLocal(product.oldPrice) }}đ
            </span>
          </div>
        </div>
      </template>
